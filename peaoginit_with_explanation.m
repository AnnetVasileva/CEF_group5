...
T1			= init+2:init+long;
% Now we run the linear regression of log(exp) = const + log(state variable ...)
% in order to account for numerical instability we leave out the last term k(T).*a(T)
% According to Marcet Hannes[1999]
X			= [ones(long-1,1) k(T) a(T) k(T).*k(T) a(T).*a(T)];
y			= log(beta*lb(T1).*(alpha*ea(T1).*ek(T1).^(alpha-1)+1-delta));
% Now we want to test whether X is well defined, for this we compute b with
% The Least Square Singular Variable Decomposition (LS SVD) and check
% whether the beta is the same as the OLS one, if the results are identical
% then our X is well-conditioned
[U,S,V] = svd(X,'econ')
btest= V*inv(S)*transpose(U)*y;
btestols = X\y;
%if the estimates are und our threshold of 0.00001 we say that they are the
%same
if abs(btest - btestols) < 0.000001
    disp(['Your X is well-conditioned '])
else 
    disp(['Your X is ill-conditioned '])
end;
%As the OLS and the LS SVD returns more or less the same estimate, we stick
%to OLS as LS SVD increases computational cost
theta		= X\y;
